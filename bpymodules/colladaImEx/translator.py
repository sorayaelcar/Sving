...
class Translator(object):
	...
	def __init__(self, isImporter, version, debugM, fileName, _useTriangles, \
				...
				_applyModifiers, _checkUnweightedVerts, _unit_name="meter", _unit_length_in_meter="1.00"):
		...
		useUV, sampleAnimation, onlyMainScene, applyModifiers, checkUnweightedVerts, \
		unit_name, unit_length_in_meter
		...
		checkUnweightedVerts = _checkUnweightedVerts
		print "check for unweighted verts = ", checkUnweightedVerts

		unit_name = _unit_name
		unit_length_in_meter= _unit_length_in_meter
...
##	  materialLibrary = None
	...
	def Export(self, fileName):
		global __version__, filename, usePhysics, unit_length_in_meter, unit_name
		filename = fileName
		...
		daeAsset.unit.name = unit_name
		daeAsset.unit.meter = unit_length_in_meter
		print "Export unit                : [", daeAsset.unit.name, "]"
		print "Export unit length in meter: [", daeAsset.unit.meter, "]"
		...
		for bScene in Blender.Scene.Get():
			if not exportCurrentScene or self.currentBScene == bScene:
				#print "export scene", bScene.name
...
##		PrintTransforms(deltaBlender, "delta")
	...
	def get_unassigned_verts_count(self, armature, mesh):
...
##							curve_yquat.append((time, pose_rotations[2]))
	...
	def BindMesh(self):
		...
		global waitingControllers, armatures, checkUnweightedVerts
...
##					if  groupName in jointBoneInfoList:
	...
	def SaveToDae(self, bModifier, bMeshObject, meshName):
		...
		if checkUnweightedVerts:
			#print "Started verts checking"
			unweightedVertsCount = self.get_unassigned_verts_count(bArmature, bMesh)
			if unweightedVertsCount > 0:
				print "Found", unweightedVertsCount, "unweighted verts in", bMesh.name
				text = str(unweightedVertsCount) + " unweighted Vertices in " + bMesh.name + "%t|Please check your weights|OK"
				Blender.Draw.PupMenu(text)
		#else:
		#	print "No verts checking to be performed for", realArmatureName


		# Get all vertextGroups
		vGroups = dict()
		for vertexGroupName in bMesh.getVertGroupNames():
			#print "Processing vgroup", vertexGroupName
