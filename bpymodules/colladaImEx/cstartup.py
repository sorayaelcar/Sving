...
# --------------------------------------------------------------------------
# History
# 2011.01.23 by Gaia Clary (The Machinimatrix):
# - Added dynamic setting of the measurements unit
# - Added a checker for unweighted vertices in weighted meshes.
...
def Main(doImp, scriptsLoc):
	...
	if debug:
		...
		transl = translator.Translator(doImport,__version__,debug,fileurl, useTriangles, usePolygons, bakeMatrices, exportSelection, newScene, clearScene, lookAt, usePhysics, exportCurrentScene, exportRelativePaths, useUV, sampleAnimation, onlyMainScene, checkUnweightedVerts, unit_name, unit_length_in_meter)
...
toggleCheckUnweightedVerts = None
toggleLookAt = None
togglePhysics = None
...
def LoadDefaultVals():
	...
	toggleUseUV, toggleOnlyMainScene, toggleApplyModifiers, toggleCheckUnweightedVerts, \
	...
	if not (colladaReg is None):
		...
		if doImport:
			...
			toggleCheckUnweightedVerts.val = colladaReg.get('checkUnweightedVerts', True)
			...
			#valueUnitName.val          = colladaReg.get('unitName', "centimeter")
			valueUnitLengthInMeter.val = colladaReg.get('unitLengthInMeter', "0.01")
...
def Gui():
	...
	toggleOnlyMainScene, toggleApplyModifiers, toggleCheckUnweightedVerts, \
	valueUnitLengthInMeter
	...
	donateText3 = "http://blog.machinimatrix.org added some minor modifications"
	...
	Blender.BGL.glRasterPos2i(45, size[1]-80)
	Blender.Draw.Text(donateText3, "small")
	...
	Blender.BGL.glRasterPos2i(45, yval+5)
	Blender.Draw.Text("Blender Unit =", "normal")

	if valueUnitLengthInMeter != None:
		valueUnitLengthInMeterVal = valueUnitLengthInMeter.val
	else:
		valueUnitLengthInMeterVal = "0.01"
    
	valueUnitLengthInMeter = Blender.Draw.String('', 18, 125, yval, 40, 20, valueUnitLengthInMeterVal, 255)
	Blender.BGL.glRasterPos2i(170, yval+5)
	Blender.Draw.Text("meter", "normal")
	...
	maxWidth = 400
	if size[0] - 125 > maxWidth:
		fileWidth = maxWidth
	else:
		fileWidth = size[0] - 125
	fileButton = Blender.Draw.String('', 5, 125, yval-5, fileWidth, 20, fileName, 255)
	
	Blender.Draw.PushButton('...', 2, 125 + fileWidth, yval-5, 30, 20, 'browse file')
	...
	if not doImport:
		...
		toggleExportSelection = Blender.Draw.Toggle('Only Export Selection',8,row1, yval, 150, 20, toggleExportSelectionVal, 'Only export selected objects')
		...
		toggleBakeMatrix = Blender.Draw.Toggle('Bake Matrices',11,row1, yval, 150, 20, toggleBakeMatrixVal, 'Put all transformations in a single matrix')
		...
		toggleSampleAnimation = Blender.Draw.Toggle('Sample Animation',16, row1, yval, 150, 20, toggleSampleAnimationVal, 'Export information for every frame of animation.')
		...
		togglePhysics = Blender.Draw.Toggle('Disable Physics',13,row1, yval, 150, 20, togglePhysicsVal, 'Disable Export physics information')


		yval = yval + 4*row_delta
		#Toggle checking for unweighted verts
		if not (toggleCheckUnweightedVerts is None):
			toggleCheckUnweightedVertsVal = toggleCheckUnweightedVerts.val
		else:
			toggleCheckUnweightedVertsVal = 0

		toggleCheckUnweightedVerts = Blender.Draw.Toggle('Check weights',14,row2, yval, 150, 20, toggleCheckUnweightedVertsVal, 'Check for unweighted verts in weighted objects')
		...
		toggleExportCurrentScene = Blender.Draw.Toggle('Only Current Scene',14,row2, yval, 150, 20, toggleExportCurrentSceneVal, 'Only Export the current scene')
		...
		toggleExportRelativePaths = Blender.Draw.Toggle('Use Relative Paths',15, row2, yval, 150, 20, toggleExportRelativePathsVal, 'Export paths relative to the collada file')
		...
		toggleUseUV = Blender.Draw.Toggle('Use UV Image Mats',15, row2, yval, 150, 20, toggleUseUVVal, 'Use UV Image instead of the material textures. Use this if you did not use the Material Textures window. Note: If you reimport this file, they will have moved to the materials section!!')
		...
		##toggleLookAt = Blender.Draw.Toggle('Camera as Lookat',14,row2, yval, 150, 20, toggleLookAtVal, 'Export the transformation of camera\'s as lookat')
		...
		toggleApplyModifiers = Blender.Draw.Toggle('Apply modifiers',14,row2, yval, 150, 20, toggleApplyModifiersVal, 'Apply modifiers, like mirroring, transformations, etc.')
...
def ButtonEvent(evt):
	...
	if evt == 1:
		...
		if valueUnitLengthInMeter is None:
			...
			unitLengthInMeter = convertToFloat(valueUnitLengthInMeter.val, 0.01)
...
def getUnitName(lengthInMeter):
	if lengthInMeter   == "0.01": unitName = "centimeter"
	elif lengthInMeter == "0.10": unitName = "decimeter"
	elif lengthInMeter == "1.00": unitName = "meter"
	else:
		unitName="undefined"
	return unitName

def convertToFloat(input, default):
	try:
		val = float(input)
	except:
		val = default
	formatted_input = "%.2f" % val
	#print "converted", input, "to", formatted_input
